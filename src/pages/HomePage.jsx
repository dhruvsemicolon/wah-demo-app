      ...
      <div className="flex flex-col lg:flex-row lg:space-x-12 py-14" >
        <div className='flex max-lg:flex-col-reverse max-lg:items-center gap-5 w-full'>
          <div className="flex lg:flex-col space-x-2 lg:space-x-0 lg:space-y-6 mb-6 lg:mb-0">
            {currentSelected?.images.map((item, index) => {
              return <div className={`w-32 h-24 border-2 ${item === currentImage ? "border-primary" : "border-gray-200 cursor-pointer"} bg-gray-200 rounded-md flex items-center justify-center`} key={index} onClick={() => { setCurrentImage(item); }}>
                <img
        ...
        <section className="flex-1 flex-shrink-0 min-w-sm space-y-4">
          ...
          <div className="flex items-center space-x-3 text-xs text-gray-500">
            ...
            <span className="text-primary font-semibold">
              ({currentSelected?.reviews} reviews)
            </span>
            <span>
              |
            </span>
            <span>
              {currentSelected?.couponsLeft} coupons left
            </span>
          </div>
          <div className="flex items-center space-x-3">
            <div className="text-primary font-semibold text-lg line-through">
              ${currentSelected?.originalPrice}
            </div>
            <div className="text-gray-900 font-bold text-xl">
              ${currentSelected?.discountedPrice}
            </div>
          </div>
          ...
          <div className="space-y-4 text-gray-600">
            ...
            <div>
              ...
              <p>
                {currentSelected?.dealTerms}
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-start sm:space-x-6 mt-6">
            <div className="text-3xl font-bold text-gray-900">
              {currentSelected?.rating}<span className="text-gray-500 text-xl">/5</span>
              <p className="text-sm text-gray-600 mt-2">{currentSelected?.reviews} Reviews</p>
              <a
                href="#"
      ...
      <div className="mt-16">
        ...
        <div className="">
          <Swiper
            ...
            slidesPerView={5}
          >
            {products.filter((v) => v.product !== currentSelected?.product).map((item, idx) => (
              <SwiperSlide
                key={idx}
